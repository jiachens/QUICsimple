...
int main(int argc, char* argv[]) {
  ...
  GURL url(urls[0]);
  cout << "First URL:" << urls[0] << "\n";
  GURL url2(urls[1]);
  cout << "Second URL:" << urls[1] << "\n";
  ...
  SpdyHeaderBlock header_block2;
  header_block2[":method"] = body.empty() ? "GET" : "POST";
  header_block2[":scheme"] = url2.scheme();
  header_block2[":authority"] = url2.host();
  header_block2[":path"] = url2.path();
  ...
  //client.SendRequest(header_block2, body, /*fin=*/true);

  // Print request and response details.
  if (!FLAGS_quiet) {
    cout << "Request:" << endl;
    cout << "headers:" << header_block.DebugString();
    cout << "headers:" << header_block2.DebugString();
    if (!FLAGS_body_hex.empty()) {
